...
unsigned long readLine(bool oledActive = false, bool debugMode = false) {
  ...
  int max_ =  sensors[0], max_index = 0;
  for (byte i = 0; i <= 15; i++)
    if (sensors[i] > max_) {
      max_ = sensors[i];
      max_index = i;
  ...
  Serial.println("Max: " + String(max_) + ", Max indx: " + String(max_index));

  long lineWidth = 0;
  // Step to right 
  for (byte i = max_index; i <= 15; i++) {
    if (sensors[i] != 0) {
      lineWidth += sensors[i];      
    } else
    break;
  ...
  Serial.println("Line Width: " + String(lineWidth));
...
/*
  int lineerPosition(unsigned int exCenterValue, int sensors[], sensorsMin[16], sensorsMax[16]) {
